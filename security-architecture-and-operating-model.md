...
An overview of `<Company Placeholder>` 's architecture can be found in [Security Architecture](). `<Company Placeholder>`  uses a highly scalable cloud architecture to provide system quality at all times.

All systems are monitored and measured in real time as described in [Application Service Event Recovery]().

`<Company Placeholder>`  uses DevOps methodology as described in [Software Development Process]() to ensure a smooth delivery process of all systems and applications.
